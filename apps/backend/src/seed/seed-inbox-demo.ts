...
/**
 * Crea (o reutiliza) una `Tag` real por cada nombre usado en el guion y devuelve el mapa
 * nombre → id. Desde HU-OMNI-04 las etiquetas son entidades del tenant, no cadenas sueltas, así
 * que el seed también las siembra para que la bandeja demo muestre chips de verdad.
 */
async function crearTagsDemo(tenantId: Types.ObjectId): Promise<Map<string, Types.ObjectId>> {
  const paleta = ['#2563EB', '#DC2626', '#EA580C', '#16A34A', '#7C3AED', '#0891B2', '#CA8A04'];
  const nombres = [...new Set(CONVERSACIONES.flatMap((c) => c.tags))];
  const mapa = new Map<string, Types.ObjectId>();

  for (const [i, nombre] of nombres.entries()) {
    await Tag.updateOne(
      { tenantId, nombre },
      { $setOnInsert: { tenantId, nombre, color: paleta[i % paleta.length] } },
      { upsert: true, collation: { locale: 'es', strength: 2 } },
    );
    const tag = await Tag.findOne({ tenantId, nombre })
      .collation({ locale: 'es', strength: 2 })
      .lean();
    if (tag) mapa.set(nombre, tag._id as Types.ObjectId);
  }
  return mapa;
}
...
async function main(): Promise<void> {
  ...
  try {
    ...
    const tagIds = await crearTagsDemo(tenantId);
    const { clientes, mensajes } = construirDocs(tenantId, asesorId, new Date(), tagIds);
