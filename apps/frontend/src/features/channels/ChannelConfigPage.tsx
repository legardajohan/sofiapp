...
function StatusBadge({ activo }: { activo: boolean }): React.ReactElement {
  return activo ? (
    <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium bg-success-subtle text-success border border-success/30">
    ...
    <div className="min-h-screen bg-background p-8">
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        <div className="flex items-start gap-3">
          <div className="flex-shrink-0 w-10 h-10 rounded-xl bg-success flex items-center justify-center">
            <svg className="w-5 h-5 text-success-foreground" fill="currentColor" viewBox="0 0 24 24">
        ...
        <div className="bg-card border border-border rounded-xl p-5 flex items-center justify-between">
          <div>
            <p className="text-sm font-medium text-foreground">Estado del canal</p>
            {status?.activo && (
              <div className="mt-1 space-y-0.5">
                <p className="text-xs text-secondary-foreground">
                  Phone Number ID: <span className="font-mono text-foreground">{status.phoneNumberId}</span>
                </p>
                <p className="text-xs text-secondary-foreground">
                  WABA ID: <span className="font-mono text-foreground">{status.wabaId}</span>
                </p>
              </div>
            )}
          </div>
        ...
        <div className="bg-card border border-border rounded-xl shadow-card">
          <div className="px-6 py-5 border-b border-border">
            <h2 className="text-base font-semibold text-foreground">Conectar canal</h2>
          </div>
          <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
            {/* WABA ID */}
            <div className="space-y-1.5">
              <label className="block text-sm font-medium text-foreground" htmlFor="wabaId">
                WABA ID
              </label>
              <input
                id="wabaId"
            ...
            {/* Phone Number ID */}
            <div className="space-y-1.5">
              <label className="block text-sm font-medium text-foreground" htmlFor="phoneNumberId">
                Phone Number ID
              </label>
              <input
                id="phoneNumberId"
            ...
            {/* Access Token */}
            <div className="space-y-1.5">
              <label className="block text-sm font-medium text-foreground" htmlFor="accessToken">
                Access Token
              </label>
              <div className="relative">
                <input
                  ...
                  placeholder="EAABwz..."
                  required
                  className="w-full px-3 py-2 pr-10 text-sm border border-input rounded-lg bg-card text-foreground placeholder-muted-foreground focus:outline-none focus:border-ring focus:ring-2 focus:ring-ring/20 transition-colors"
                />
                <button
                  type="button"
