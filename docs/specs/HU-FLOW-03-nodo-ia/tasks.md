...
- [x] `features/flow/flow.types.ts`: `'ia'` en `TIPOS_NODO`; interfaz `ISalidaIa`; rama `ia` de
      `ConfigNodo` (`objetivo`, `salidas`, `ramaPorDefecto`, `maxTurnos`, `usarKb`).
- [x] `features/flow/flow.types.ts`: `RequiereMotor` gana
      `{ tipo: 'ia'; objetivo; salidas; usarKb }` y `EntradaMotor.resueltos` gana
      `ia?: { respuesta: string; salida: string | null }`.
- [x] `features/flow/flow.model.ts`: `'ia'` en el `enum` de `tipo` del `NodoSchema`.
- [x] `features/flow/flow.validation.ts`: rama `.strict()` de `ia` con `maxTurnos` entre 1 y 10 y
      `salidas` entre 1 y 8.
- [x] `features/flow/flow.validation.ts` (`superRefine` del grafo): `ia` se suma a los tipos que
...
- [x] `features/flow/flow.engine.ts`: `case 'ia'` con las tres situaciones **en este orden** —
...
- [x] Contador de turnos en `variables` bajo `_ia:<nodoId>:turnos`, y **borrado** del objeto de
      variables al salir del nodo por cualquier rama. Sin campo nuevo en `FlowState`.
- [x] Al salir por una rama, el nodo **no** emite `enviar_mensaje`: habla el nodo destino.

### 3. Runtime

- [x] `flow.runtime.service.ts`: `construirHistorial(tenantId, clienteId, limite)` — `findScoped`
...
- [x] `flow.runtime.service.ts`: usar `construirHistorial` **solo** en el caso `'ia'`.
      `intencion`, `kb` y `captura` conservan su historial de un turno.
- [x] `flow.runtime.service.ts`: `resolverRequiere` case `'ia'` — **una sola** llamada a
      `ai.extract()` con dos `SlotSpec` (`respuesta`, `salida`) y schema
      `{ respuesta: string; salida: enum | '' → null }`.
- [x] Con `usarKb: true`, anteponer al historial un turno con los fragmentos de
...
- [x] `features/flows/types.ts`: espejo exacto de `TIPOS_NODO` y de la rama `ia` de `ConfigNodo`.
- [x] `components/nodeVisuals.ts`, los cinco puntos: `NODE_VISUALS.ia` (`Sparkles`,
...
- [x] `components/IaOutputsEditor.tsx`: lista de salidas (etiqueta + descripción + `DestinoSelect`),
      añadir y quitar, sobre el patrón ya probado de las etiquetas de `intencion`.
- [x] `components/NodeInspector.tsx`: `<IaForm>` (objetivo, `usarKb`, `maxTurnos`, salidas,
      rama por defecto) + su rama en el render por `nodo.config.tipo`.
- [x] Texto largo con el afordance "Ver más" de `HU-FLOW-01-V3` en las descripciones de las salidas;
      nada de scroll horizontal.
- [x] El panel explica que **al elegir una salida, el mensaje lo da el nodo siguiente** — es la duda
      que tiene todo el que dibuja este nodo por primera vez.
- [x] "Máximo de turnos" acompañado de su coste ("cada turno es una consulta al modelo"), no un
      número desnudo.
- [x] Reusar `textarea`, `input`, `switch`, `select` del UI kit; nada hecho a mano.
- [x] Revisar en **light y dark** con tokens semánticos; cero `bg-[#...]`.
- [x] `AddNodeMenu.tsx` **no** se toca (itera `TIPOS_NODO`); confirmar que el tipo aparece solo.

## Tests (Vitest)

### Aislamiento multi-tenant (obligatorio)

- [x] `flow.isolation.test.ts` (ampliar): con dos tenants que tienen un cliente cada uno,
      - [x] `construirHistorial(tenantA, clienteDeB)` devuelve vacío: nunca mensajes de B.
      - [x] La llamada a `AIService` de un nodo `ia` del tenant A lleva el `tenantId` de A, de modo
            que la plantilla de prompt, la clave de caché y la KB consultadas son las de A.
      - [x] `searchKnowledge` se invoca con el `tenantId` de la conversación, nunca con otro.

### Motor puro — `flow.engine.test.ts`

- [x] Sin `resueltos.ia`: devuelve `requiere: { tipo: 'ia', ... }` y no emite efectos.
- [x] Con `salida` que coincide con una etiqueta: avanza al `nodoDestino` de esa salida
      (**criterio 2, el DoD de la historia**) y **no** envía la respuesta de ese turno.
- [x] Con `salida` que no coincide con ninguna etiqueta: avanza por `ramaPorDefecto`.
- [x] Con `salida: null`: emite `enviar_mensaje` con la respuesta, se queda en el nodo y deja
      `esperandoRespuesta: true` (criterio 3).
- [x] El contador `_ia:<nodoId>:turnos` sube un turno por cada respuesta y **desaparece** de
      `variables` al salir del nodo por cualquier rama.
- [x] Alcanzado `maxTurnos`: sale por `ramaPorDefecto` y **no** devuelve `requiere` (criterio 4).
- [x] Un `resueltos.ia` que llega justo con el contador en el tope se atiende igual: no se descarta
      una respuesta ya generada.

### Validación — `flow.validation.test.ts`

- [x] `maxTurnos: 0`, `maxTurnos: 11` y `salidas: []` → 400.
- [x] Una clave extra en el `config` de un nodo `ia` → 400 (`.strict()`).
- [x] Un `nodoDestino` de salida que no existe → 400.
- [x] Un nodo alcanzable **solo** desde una salida de `ia` no se reporta como huérfano (criterio 5).

### Nodo `ia` de punta a punta — `flow.ia-node.test.ts`

- [x] Con `AIService` mockeado devolviendo primero `salida: null` dos veces y luego una etiqueta:
      el flujo envía dos mensajes, se queda en el nodo entre medias y termina en el nodo destino.
- [x] El historial que recibe el mock contiene los mensajes previos de la conversación en orden
      cronológico, no solo el último (criterio 3).
- [x] Con `usarKb: false` no se llama a `searchKnowledge`; con `true`, sí, y sus fragmentos llegan
      al historial.
- [x] `iaHabilitada: false` en el cliente → el nodo no responde ni llama a la IA (criterio 6).
- [x] Cuota agotada → `sendOutbound` lanza, se registra y el flujo no se cae (criterio 7).

## Documentación

- [x] `docs/domain.md` y `docs/data-model.md`: el nodo `ia` en el vocabulario de tipos de nodo, con
      su `config`.
- [x] `docs/integrations/llm-provider.md`: el nodo `ia` como consumidor de `AIService.extract()` y
...
- [ ] **Prueba manual E2E:** añadir un nodo `ia` con dos salidas al flujo demo de
      ...
      rama correcta. _Pendiente: verificación en vivo la hace el usuario, no Claude Code (ver
      convención vigente del proyecto)._
- [ ] Las capturas de verificación quedaron en `.playwright-mcp/` o el scratchpad, y se borraron.
      _N/A: no se generaron capturas — no hubo verificación visual en esta sesión._
