# HU-FLOW-01-V2 — Plan técnico (CÓMO)

> Los contratos técnicos (tipos, motor, persistencia, runtime, endpoints) son los de
> `docs/specs/HU-FLOW-01-condiciones-respuesta/plan.md`, verificados contra el estado actual del
> código por exploración dedicada — no cambian en V2. Este documento los reproduce completos y
> añade la sección de UX ampliada que sí es nueva (criterios 16–23 de `spec.md`).
...
├── components/EmptyFlowState.tsx       # NUEVO V2: estado vacío guiado (criterio 16)
├── components/ActivateFlowDialog.tsx   # NUEVO V2: AlertDialog de confirmación (criterio 18)
...
└── index.css                           # variables CSS de @xyflow/react alineadas a la paleta
docs/
...
## Frontend — UX/UI (criterios 16–23 de `spec.md`, el foco explícito de V2)

Invocar `emil-design-eng`, `impeccable:impeccable` y `frontend-design:frontend-design` **antes** de
escribir el primer componente (regla §7 del `CLAUDE.md` raíz). Mapeo criterio → implementación
concreta:

| Criterio | Componente | Decisión |
|---|---|---|
| 16 — estado vacío guiado | `EmptyFlowState.tsx` | Se muestra en `FlowEditorPage` cuando `nodos.length === 0`: mensaje corto + botón que crea el nodo `entrada` en el centro del canvas. Reemplaza el canvas en blanco, no lo superpone. |
| 17 — badge de flujo activo | `FlowsPage.tsx` | `Badge` variant `success` junto al nombre del flujo con `activo: true`; el resto sin badge (no un badge "inactivo" que compita visualmente). |
| 18 — confirmación de activación | `ActivateFlowDialog.tsx` (`AlertDialog` de shadcn) | Se dispara al marcar `activo: true` sobre un flujo cuando ya existe otro activo; el texto nombra el flujo que se va a desactivar, no un genérico "¿Estás seguro?". |
| 19 — estado de carga al guardar | `FlowEditorPage.tsx` | El botón "Guardar" queda `disabled` + spinner mientras la mutación de `useSaveFlow` está `isPending`; en `onError` el store de Zustand conserva `nodos`/`aristas` tal como estaban (la mutación nunca limpia el canvas). |
| 20 — nodo `handoff` distinguible | `components/nodes/HandoffNode.tsx` | Color e icono propios (p. ej. `border-destructive` + icono de traspaso), distinto del resto de nodos custom. |
| 21 — errores anclados al nodo | `NodeInspector.tsx` + `FlowCanvas.tsx` | La respuesta 400 de Zod trae `path` con el `id` del nodo; el canvas resalta ese nodo y el inspector abre directamente en él con el mensaje, no un toast suelto. |
| 22 — inspector no tapa el canvas | `FlowEditorPage.tsx` (layout) | Grid de dos columnas con el inspector en un panel de ancho fijo (`w-80`/`w-96`), nunca superpuesto (`absolute`) sobre el canvas, verificado a 1280px. |
| 23 — light y dark completos | Todo lo anterior + `index.css` | Cero `bg-[#...]`; variables CSS de `@xyflow/react` alineadas a la paleta del proyecto (ver abajo). |

Otros puntos de diseño ya decididos en la iteración V1 y que se mantienen:
...
- **Tipos de nodo distinguibles sin leer**: color e icono por tipo, con los tokens semánticos.
...
Componentes shadcn/ui: `card`, `select`, `input`, `button`, `badge`, `dialog`, `alert-dialog`,
`tooltip`, `separator`, `sheet` (ya vendorizados). Probablemente falten `tabs` y `popover`:
...
- **Patrón tenant-safe a copiar**: `apps/backend/src/features/tag/*` (usa `*Scoped` en todos lados).
  **No** copiar `apps/backend/src/features/channel/channel.service.ts` — accede a Mongoose directo,
  es una desviación documentada del proyecto, no el patrón canónico.
- **Patrón Zustand+TanStack a copiar en frontend**: `apps/frontend/src/features/admin-plans/` (store
  de UI separado de las queries/mutations del servidor).

## Verificación

- `pnpm --filter backend typecheck` · `pnpm --filter backend test`
- `pnpm --filter frontend build && pnpm --filter frontend lint`
- **La prueba E2E manual (flujo con dos ramas, mensaje real por cada rama) y el recorrido visual de
  los 8 criterios UX (16–23) en light/dark los hace el usuario** — sin Playwright ni verificación
  visual de parte de Claude.
