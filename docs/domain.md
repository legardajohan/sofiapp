...
| **Flujo** | Grafo de conversación automatizada (constructor visual, `HU-FLOW-01-V2`). Nodos: `mensaje`, `captura`, `condicion`, `intencion`, `kb`, `accion`, `handoff`, `espera` (este último lo ejecuta `HU-FLOW-02`). **No almacena conocimiento**: el nodo `kb` delega en `AIService.chat()` y los nodos `condicion`/`intencion` no admiten texto de respuesta propio — esa regla es estructural (Zod la exige), no una convención. El estado de ejecución por conversación vive en `FlowState`, una colección propia (no un campo más de `Cliente`). |
