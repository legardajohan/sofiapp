...
| **Tipo de mensaje** | `texto \| enlace \| imagen \| video \| audio \| documento \| sticker \| plantilla \| otro` (`HU-OMNI-06`). **`enlace` es un mensaje CON texto**: es un `texto` que contiene una URL, se clasifica aparte solo para poder pintar la tarjeta de previsualización. Toda decisión de «¿este mensaje se puede leer?» pasa por `esTipoConTexto()`; preguntar `tipo !== 'texto'` haría que compartir un link disparase el acuse de «solo entiendo texto». |
| **Media** | Archivo adjunto a un mensaje, con ciclo propio: `pendiente` → `disponible` \| `fallida`. La descarga desde Meta es asíncrona (cola `media-ingest`), así que el mensaje aparece en la bandeja antes que su archivo. El modelo guarda una **clave de almacenamiento**, nunca una URL: la del hilo se firma en cada lectura (ADR-0008). |
...
`CatalogItem`, `Campaign`, `AuditEvent`, `Flow`, `FlowState`. Esquemas en `data-model.md`.
...
Todo lo de esta sección es del eje **conversación** de la tabla de arriba: la IA mueve la etiqueta
del hilo, nunca el campo `Lead.semaforo`. Cambiar el semáforo de la oportunidad sigue siendo un acto
humano (`PATCH /api/leads/:id/status`).
