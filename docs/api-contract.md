...
| GET | `/api/estados` | admin | Catálogo de etapas del pipeline del tenant, en orden. Incluye las archivadas (`activo: false`), que ya no se ofrecen para filtrar pero siguen resolviendo su etiqueta en los leads que las llevan (HU-CRM-03). Con `?uso=true` cada etapa añade `leads`: cuántos leads la tienen grabada — lo pide la pantalla de gestión para explicar por qué una etapa no se puede eliminar; sin el parámetro el campo **no viaja** (ausente ≠ cero). |
...
| PATCH | `/api/estados/orden` | admin | Reordena el embudo: `{ ids }` con **todas** las etapas del tenant (activas y archivadas) en el orden deseado; el servidor asigna `orden = 0..n-1`. Se exige una permutación exacta del catálogo — una lista incompleta dejaría a las que faltan con su posición vieja colándose en el recorrido nuevo, y una con ids de más significa que el cliente vio un catálogo que ya no existe → `409`, recargar. Registrada **antes** de `/:id` en el router. Responde el catálogo ya ordenado. |
| PATCH | `/api/estados/:id` | admin | Edita una etapa: `{ label?, color?, orden?, activo?, esSalida? }`, al menos un campo. La `key` **no** es editable: es el valor grabado en `Lead.estado`, así que renombrar conserva el vínculo con sus leads. `activo: false` **archiva** (sale del tablero y de los filtros, sigue resolviendo su nombre) y `true` la recupera. Nombre repetido → `409`. Archivar la etapa de entrada (`nuevo`) → `409` con `details.motivo: 'entrada'`: los leads convertidos nacen en ella y quedarían fuera del tablero. |
| DELETE | `/api/estados/:id` | admin | Borra una etapa, **solo si ningún lead está en ella** → `204`. Con leads → `409` y `{ motivo: 'en_uso', enUso: <n>, key }`: el borrado se rechaza, **no se archiva por su cuenta** (archivar es el `PATCH` de arriba, que el administrador decide en el diálogo). La etapa de entrada (`nuevo`) → `409` con `{ motivo: 'entrada' }` aunque esté vacía. |
...
| GET | `/api/pipeline` | admin | El embudo agrupado por etapa (HU-PIPE-01): `{ columnas: [{ etapa, total, leads }], limit }`, una columna por etapa **activa** en orden de pipeline. Una etapa sin leads aparece igual con `total: 0`. Filtros: `?asesor`, `?semaforo`, `?desde`, `?hasta` (los mismos del listado) y `?limit=` **por columna** (default 20, máx 50). **No admite `?estado=`** → `400`: el tablero ya agrupa por etapa. Cada columna trae su primera página; recorrer el resto es trabajo de `GET /api/leads`. Las tarjetas comparten la proyección de `GET /api/leads`, así que heredan sus dos reglas: el `semaforo` es el **campo del lead** (HU-CRM-04) y el `resumen` lleva el mismo gate de datos sensibles — solo subrol `director`/`manager` o `admin` sin subrol; para el resto llega `null` (HU-IA-04). |
...
| POST | `/api/campaigns/segmento/preview` | admin | Cuánta gente cae en un segmento y cuánto cupo hay hoy: `{ filtros }` → `{ total, muestra[], presupuesto }` (HU-MARK-01). Los filtros combinan en AND `atributos` (`{ key, valores[] }` — por aquí entra «grado», que **no** es un campo del modelo sino un atributo personalizado de HU-CRM-02), `rolContacto`, `semaforoLead` (key del catálogo `semaforos`, el eje **comercial** del lead), `nivelInteres`, `estadoComercial` y `tagIds`. Una clave que no exista en el tenant → **segmento vacío**, no `400`. Los contactos con `marketingOptOut` se excluyen **siempre**. Registrada ANTES de `/:id` en el router: es ruta literal. |
| GET | `/api/campaigns` | admin | Listado paginado de campañas (`?page&limit&estado`), `createdAt` descendente (HU-MARK-01). |
| POST | `/api/campaigns` | admin | Crea una campaña → `201`. `{ nombre, filtros, templateId, parametros[], lanzar?, programadaPara? }`, schema `.strict()`. `lanzar` y `programadaPara` son **excluyentes** (`400`: un cuerpo con las dos describe dos momentos de arranque); `programadaPara` en el pasado → `400`. Con `lanzar: true` arranca en el acto y consume la cuota `campanasMes` (`429` al límite). Plantilla no `APPROVED` → `422`; número de parámetros distinto al del cuerpo → `400` con `{ esperados, recibidos }`. Calidad del número en `RED` o cupo de 24 h agotado → `409`. Segmento sin nadie → `422`. Al lanzar, el segmento se **congela**: `totales.destinatarios` queda fijo. |
| GET | `/api/campaigns/:id` | admin | Detalle con `totales`, `presupuesto` congelado, la plantilla resuelta y el `desglose` por estado de destinatario (los cinco siempre presentes, con `0`). Campaña de otro tenant → `404`, nunca `403`. |
| GET | `/api/campaigns/:id/destinatarios` | admin | Destinatarios paginados, `?estado=`. Id de otro tenant → `404`, **no** una página vacía. |
| POST | `/api/campaigns/:id/launch` | admin | Lanza un borrador. **Idempotente**: relanzar algo ya en curso o completado devuelve la campaña sin duplicar destinatarios ni volver a consumir cuota. Una campaña `cancelada` no se relanza → `409`. |
| POST | `/api/campaigns/:id/pause` · `/resume` · `/cancel` | admin | Transiciones, body vacío `.strict()`. Pausar algo que no está en curso → `409`. Reanudar revalida el cupo (`409` si el número está en rojo). Cancelar deja los pendientes en `omitido` —no en `fallido`: a esa gente no se le llegó a escribir— y no se puede deshacer. **No hay `DELETE`**: una campaña es un hecho ocurrido. |
| POST | `/api/channels/whatsapp/tier/sync` | admin | Refresca `messagingTier`, `qualityRating` y `healthStatus` desde la Graph API (HU-MARK-01). Si la sonda falla **no lanza**: conserva lo guardado, para que un token caducado no impida lanzar una campaña. Respeta el override manual. |
| PATCH | `/api/channels/whatsapp/tier` | admin | Override manual del tier y/o la calidad (`{ messagingTier?, qualityRating? }`, al menos uno). Marca `tierManual`, y desde entonces la sonda ya no pisa el tier. Existe porque la Graph API devuelve `403` en escenarios legítimos (token de dashboard caducado, número sandbox). |
